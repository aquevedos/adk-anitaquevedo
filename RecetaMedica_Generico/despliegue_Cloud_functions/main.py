...
# Inicializar colección de usuarios en Firestore
...
def procesar_receta_http(request):
    ...
    if 'action' in request.form and request.form['action'] == 'login':
        ...
        if autenticar_usuario(username, password):
            ...
            headers_cookie = {
                ...
                'Set-Cookie': f'session_token={session_val}; HttpOnly; Secure; SameSite=Lax; Path=/'
            }
    ...
    if 'action' in request.form and request.form['action'] == 'register':
        ...
        if not password or len(password) < 6:
            return (json.dumps({"status": "error", "message": "La contraseña debe tener al menos 6 caracteres."}), 400, headers)
        if registrar_usuario(username, password):
            return (json.dumps({"status": "success", "message": "Registro completado con éxito. Ahora puede iniciar sesión."}), 200, headers)
        else:
            return (json.dumps({"status": "error", "message": "El usuario ya existe o no pudo ser registrado."}), 400, headers)
